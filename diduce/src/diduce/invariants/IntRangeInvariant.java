...
public class IntRangeInvariant extends diduce.invariants.Invariant implements java.io.Serializable {
    ...
    else
    {
        if (x < _min)
        {
            if (diduce.Runtime._check_violations)
            {
                ...
                Relaxation r = new Relaxation (spp_descriptor, conf_change, "Min " + invariant_name, range_str, Integer.toString(x), _nSamples-1);
             }
        ...
        else if (x > _max)
        {
            if (diduce.Runtime._check_violations)
            {
                ...
                Relaxation r = new Relaxation (spp_descriptor, conf_change, "Max " + invariant_name, range_str, Integer.toString (x), _nSamples-1);
             }
